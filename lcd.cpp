...
void lcd_setCursor(uint8_t row, uint8_t col)
{
	...
	{
		case 1:
			sendLcd_cmd((col|=0x80));
		break;
		case 2:
			sendLcd_cmd((col|=0xC0));
		 break;
		//case 3:
		//	sendLcd_cmd((col|=0x94));
		// break;
		//case 4:
		//	sendLcd_cmd((col|=0xD4));
		// break;
	}
}
...
{
	//for cmd mode RS must be 0 and RW must be 0 for write operation
	GPIOA->ODR &=~(1<<RS);
	//RW is connected to GND
	//1 - HIGH nible
	int cmd1=cmd>>4;
